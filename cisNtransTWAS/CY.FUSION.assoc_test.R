...
option_list = list(
  ...
  make_option("--ref_ld", action="store", default=NA, type='character',
              help="Prefix to reference LD files in binary PLINK format aggregates all 22 chromosomes [required]"),
  make_option("--weightTEMPdir", action="store", default=NA, type='character',
              help="Path to directory where temporary plink file for weight calculation on variants on multi-chromosomes per feature are stored [required]"),
...
#######################
## CY note!!! on 1/6/2023: 
## "sumstat.full" need to be assigned here, otherwise the "overwriting" sumstat same object would mess all the rest features up while looping!!!
## see the lines 180-188 for the sections mentioning this issue
sumstat.full = read.table(opt$sumstats,head=T,as.is=T)
...
# Load in the bim file for the full-22chr reference data
root <- opt$ref_ld
proot <- path.expand(root)
bimfile <- paste(proot, ".bim", sep = "")
all.geno.bim <- fread(bimfile, header = FALSE)
...
for ( w in 1:nrow(wgtlist) ) {
  cat( w, 'starts', '\n' )
	...
	# Match up the SNPs and weights first time
	m1 = match( snps[,2] , all.geno.bim[V1 %in% unique(snps$V1)]$V2 )  ## CY changed genos here to the multi-chr bim variants
	m.keep1 = !is.na(m1)
	snps = snps[m.keep1,]
	wgt.matrix = wgt.matrix[m.keep1,,drop=F]
	
	cat('chrs:', unique(snps$V1), 'for', wgtlist[w,3], '\n')
	###########################################
  ## CY note on 1/5/2023: temp files for reading as genos here given the "chr"s from the weights cannot be generated not automatically recognize how many chromosomes need to be added
	## thus I turned to the temp file I created in the weight-calculation step using the SNP-matching list extraction instead
	##################################
	## TODO: move this per protein to accommodate the multi-chromosome situations
	# Load in reference data
	genos = read_plink(paste(opt$weightTEMPdir, '/temp_', wgtlist$ID[w],sep=''),impute="avg")
	
	#######################
	## CY note!!! on 1/6/2023: 
	## "sumstat.full" need to be on the right, otherwise the "overwriting" sumstat same object would mess all the rest features up while looping!!!
	
	m2 = match( genos$bim[,2] , sumstat.full$SNP )
	# m.keep2 = !is.na(m2)
	sum.missing = is.na(m2)
	## "sumstat.full" need to be on the right, see above note
	sumstat = sumstat.full[m2,]
	...
	# Match up the SNPs and weights again to reduce the size of the genotype-matrix
	...
	# system(paste0('rm ', opt$temp_dir, '/', wgtlist$ID[w], '_temp.*')) # not used as I used the weight-calculation temp folder instead of the multi-chr temp creation
}
...
# WRITE MHC TO SEPARATE FILE [also updated by CY on 1/5/2023]
